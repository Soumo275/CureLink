...
const CartPage = () => {
    ...
    const handleIncreaseQuantity = (medicine) => {
        dispatch(updateQuantity({ _id: medicine._id, quantity: medicine.quantity + 1 }));
    };
                        ...
                        {cartItems.length > 0 ? (
                            <ul role="list" className="-my-6 divide-y divide-gray-200">
                                {cartItems.map((medicine) => (
                                    <li key={medicine._id} className="flex py-6">
                                        <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                                            <img
                                                alt={medicine.name}
                                                src={`${getImgUrl(medicine.coverImage)}`}
                                        ...
                                        <div className="ml-4 flex flex-1 flex-col">
                                            <div>
                                                <div className="flex flex-wrap justify-between text-base font-medium text-gray-900">
                                                    ...
                                                    <p className="sm:ml-4">Rs.{medicine.newPrice}</p>
                                                </div>
                                            ...
                                            <div className="flex flex-1 flex-wrap items-end justify-between space-y-2 text-sm">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <button
                                                        onClick={() => handleDecreaseQuantity(medicine)}
                                                    ...
                                                    <button
                                                        onClick={() => handleIncreaseQuantity(medicine)}
                                                        className="bg-gray-200 p-2 rounded-full text-lg"
                                                    >
                                                        +
                                                    </button>
                            ...
                            <p>No medicines found in your cart!</p>
                        )}
                    </div>
                </div>
            </div>
